...
export default function RetailerNotifications() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get(`/api/notifications/${retailerId}`)
    ]).then(([res, stateRes]) => {
  ...
  const markRead = async id => {
    try {
      await api.patch(`/api/notifications/${retailerId}/${id}`, { isRead: true });
  ...
  const markAllRead = async () => {
    try {
      ...
      await api.post(`/api/notifications/${retailerId}/mark-all-read`, { notificationIds: unreadIds });
  ...
  const del = async id => {
    try {
      await api.patch(`/api/notifications/${retailerId}/${id}`, { isDeleted: true });
